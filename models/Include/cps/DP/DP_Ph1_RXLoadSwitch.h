...
namespace Ph1 {
	...
	public:
		...
		public:
			MnaPreStep(RXLoadSwitch& load) : Task(load.mName + ".MnaPreStep", load.mSubsystem), mLoad(load) {
		...
		public:
			MnaPostStep(RXLoadSwitch& load, Attribute<Matrix>::Ptr leftVector) :
				Task(load.mName + ".MnaPostStep", load.mSubsystem), mLoad(load), mLeftVector(leftVector) {
